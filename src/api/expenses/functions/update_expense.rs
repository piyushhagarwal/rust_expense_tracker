...
pub async fn update_expense_function(pool: &PgPool, user_id: i32, amount: f64, expense_name: String, expense_category: String) -> sqlx::Result<()>{
...
mod tests {
    ...
    async fn test_add_expense(){
        ...
        update_expense_function(&pool, 1, 200.0,"transportation".to_string(),"Travel".to_string()).await.unwrap();
    }
}